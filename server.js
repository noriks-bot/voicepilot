...
// ═══════════════════════════════════════════════════════════════════════════
// LipVoiceMaker (v2)  —  EN -> HR (test)
// 1 zvok iz videa  2 klon glasu govorca (ali rezerva)  3 STT z besednimi casi
// 4 GPT: znamke -> NORIKS + prevod  5 TTS s klonom  6 obraz? -> lipsync
// 7 VEED-slog podnapisi (NA KONCU)
// Voiceover Maker se NE dotakne — vse je pod /api/lipvoice/*
// ═══════════════════════════════════════════════════════════════════════════
const LV_DIR = path.join(__dirname, 'lipvoice');
const LV_UP = path.join(LV_DIR, 'uploads'), LV_OUT = path.join(LV_DIR, 'out');
[LV_DIR, LV_UP, LV_OUT].forEach(d => { if (!fs.existsSync(d)) fs.mkdirSync(d, { recursive: true }); });
...
const lvUpload = multer({
    storage: multer.diskStorage({
        destination: (q, f, cb) => cb(null, LV_UP),
...
async function lvSTT(audioPath, job, langHint) {
    ...
    try {
        ...
        lvLog(job, 'STT: ElevenLabs ' + r.status + ' -> rezerva Whisper');
    } catch (e) { lvLog(job, 'STT ElevenLabs napaka -> rezerva Whisper'); }
...
async function lvClone(audioPath, job) {
    ...
    } catch (e) { lvLog(job, 'klon glasu napaka -> rezervni glas'); }
    return null;
}
...
async function lvTranslate(segs, lang, product, job) {
    const FULL = { HR: 'Croatian', SK: 'Slovak', SI: 'Slovenian', CZ: 'Czech', PL: 'Polish', HU: 'Hungarian', RO: 'Romanian', GR: 'Greek', DE: 'German' };
...
6. Output ONLY the numbered lines.`;
    const r = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({ model: 'gpt-4o', temperature: 0.4, messages: [
            ...
            { role: 'user', content: segs.map((s, i) => `${i + 1}. ${s.text}`).join('\n') }] })
    });
    if (!r.ok) throw new Error('GPT ' + r.status);
    const _gj = await r.json();
...
[V4+ Styles]
Format: Name,Fontname,Fontsize,PrimaryColour,SecondaryColour,OutlineColour,BackColour,Bold,Italic,Underline,StrikeOut,ScaleX,ScaleY,Spacing,Angle,BorderStyle,Outline,Shadow,Alignment,MarginL,MarginR,MarginV,Encoding
Style: V,DejaVu Sans,${fs_},&H00FFFFFF,&H00FFFFFF,&H00000000,&H00000000,-1,0,0,0,100,100,0,0,1,${Math.round(fs_*0.11)},0,2,${Math.round(W*0.07)},${Math.round(W*0.07)},${mv},1
...
`;
    ...
    for (const s of segs) {
        ...
        if (!txt) continue;
        const ws = (s.words || []).filter(w => w.w);
        if (ws.length >= 2 && ws.length <= 14) {
            // poudarek po besedah (karaoke) — kot VEED
            const parts = ws.map(w => esc(w.w));
            for (let i = 0; i < ws.length; i++) {
                const line = parts.map((p, k) => k === i ? `{\\c${HL}}${p}{\\c&H00FFFFFF&}` : p).join(' ');
                a += `Dialogue: 0,${t(ws[i].s)},${t(ws[i].e)},V,,0,0,0,,{\\fad(60,60)}${line}\n`;
            }
        } else {
            a += `Dialogue: 0,${t(s.s)},${t(s.e)},V,,0,0,0,,{\\fad(120,120)}${txt}\n`;
        }
    }
    return a;
}
...
async function lvHasFace(video, work, job) {
    try {
        ...
        const imgs = files.slice(0, 6).map(f => ({ type: 'image_url',
...
async function lvLipsync(videoIn, audioIn, outPath, job, lang, resizeFactor, onPct) {
    ...
    const p = execPromise(
        ...
        `--resize_factor ${rf} --pads 0 15 0 0 --wav2lip_batch_size 64 --face_det_batch_size 8 > '${lvSh(logf)}' 2>&1`,
    ...
    const iv = setInterval(() => {
        try {
            ...
            if (typeof onPct === 'function') onPct(faza === 'detekcija obraza' ? (+pct) / 2 : 50 + (+pct) / 2);
...
app.get('/api/lipvoice/download/:id/:lang', (req, res) => {
    ...
    if (!f || !fs.existsSync(f)) return res.status(404).send('Ni datoteke');
    res.download(f, `${j.name}-${L}.mp4`);
});
// ═══════════════════ END LipVoiceMaker (v2) ═══════════════════
